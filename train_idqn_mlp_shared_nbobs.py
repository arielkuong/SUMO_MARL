...
@torch.no_grad()
def evaluate_idqn_mlp_shared_neighbor_obs(
    args,
    shared_q_network: QNetMLP,
    agent_id_list: List[str],
    neighbor_map_template: Dict[str, List[str | None]],
    run_name: str,
) -> float:
    ...
    try:
        ...
        for eval_env in eval_env_pool.envs:
            ...
            # Rebuild neighbor map for THIS env's TLS ordering (ids should match format "n_i_j")
            eval_agent_ids = list(eval_env.agent_ids)
            eval_neighbor_map = build_neighbor_map(eval_agent_ids)

            while not done_flag:
                # Augment with neighbor summaries
                aug_obs_dict = augment_obs_with_neighbors(observation_dict, eval_agent_ids, eval_neighbor_map)
                obs_matrix = np.stack([aug_obs_dict[aid] for aid in eval_agent_ids], axis=0).astype(np.float32)

                obs_tensor = torch.as_tensor(obs_matrix, dtype=torch.float32, device=device)  # [N, obs_aug]
                q_values = shared_q_network(obs_tensor)                                      # [N, A]
                greedy_actions = torch.argmax(q_values, dim=-1).tolist()
                action_dict = {aid: int(greedy_actions[i]) for i, aid in enumerate(eval_agent_ids)}
...
def run_training(args):
    ...
    # Build neighbor map ONCE for training TLS ids
    neighbor_map = build_neighbor_map(agent_id_list)

    # Work out augmented obs dim
    # (apply augmentation once on the initial obs)
    ...
    shared_q_network = QNetMLP(augmented_observation_dim, action_dim, hidden_units=args.hidden).to(compute_device)
    target_q_network = QNetMLP(augmented_observation_dim, action_dim, hidden_units=args.hidden).to(compute_device)
    ...
    run_name = "dqn_mlp_shared_neighbor_obs"
    ...
    for episode_idx in range(1, args.episodes + 1):
        ...
        while not episode_done_flag and steps_this_episode < args.episode_steps:
            ...
            obs_matrix = np.stack([aug_obs_dict[aid] for aid in agent_id_list], axis=0).astype(np.float32)

            obs_tensor = torch.as_tensor(obs_matrix, dtype=torch.float32, device=compute_device)  # [N, obs_aug]
            with torch.no_grad():
            ...
            # Push each junction's transition into the shared replay buffer (with AUGMENTED states)
            ...
            for i, agent_id in enumerate(agent_id_list):
                transition = Transition(
                    state=aug_obs_dict[agent_id],
                    action=int(chosen_actions_np[i]),
                    reward=float(reward_dict[agent_id]),
                    next_state=aug_next_obs_dict[agent_id],
        ...
        if total_env_steps_collected >= args.warmup_steps:
            ...
            if episode_idx % args.eval_every == 0:
                ...
                evaluate_idqn_mlp_shared_neighbor_obs(args, shared_q_network, agent_id_list, neighbor_map, run_name=run_name)
...
def parse_args():
    parser = argparse.ArgumentParser("IDQN (MLP, SHARED) + 1-hop neighbor-augmented obs — Train on Random, Eval on Fixed")
