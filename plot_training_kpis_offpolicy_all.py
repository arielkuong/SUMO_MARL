...
def main():
    ...
    parser.add_argument("--eval-every", type=int, default=5, help="Episodes per evaluation (fixed for all runs).")
    parser.add_argument("--max-points", type=int, default=150, help="Max evaluation points per run.")
    ...
    # Legend grouping
    memoryless_labels = ["IDQN (MLP)", "DQN (GNN)", "DQN (CTDE+MLP)", "DQN (CTDE+GNN)"]
    lstm_labels       = ["IDRQN (LSTM)", "DRQN (GNN+LSTM)", "DRQN (CTDE+LSTM)", "DRQN (CTDE+GNN+LSTM)"]
