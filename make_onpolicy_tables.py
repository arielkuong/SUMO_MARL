...
def compute_kpi_widths_ex(
    ...
    # New heuristics: TP wider; MTT narrower; AWT narrowest
    tp_scale, tp_pad, tp_min = 0.75, 2, 9   # wider than before
...
def inner_tabular_with_grid(
    ...
    # Make grid column a bit narrower
    grid_box_w_ex = 4
...
def build_2x2_single_table(
    ...
    lines: List[str] = []
    lines.append("% =============================================")
    lines.append("% One table: 2×2 grid of on-policy results (mean ± std) with fixed KPI widths")
    lines.append("% =============================================")
    ...
    lines.append("\\caption{Performance of on-policy (A2C-based) MARL policies across representative traffic demand scenarios. "
...
def main():
    ap = argparse.ArgumentParser(
        "Build ONE LaTeX table (2×2 grid) for on-policy evaluations from CSVs (mean ± std inside each CSV)."
    )
