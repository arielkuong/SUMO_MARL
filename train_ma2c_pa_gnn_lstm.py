...
from marl_utils.network_update import ma2c_pa_update_gnn_lstm_attn
from marl_utils.common import (
    set_global_seed,
    EvalHistory,
    clear_eval_history,
    build_grid_edge_index,   # helper that builds [2,E] from agent ids
)
from env_builder import build_train_env, get_or_create_eval_pool


# ------------------ evaluation (greedy; recurrent actor) ------------------
@torch.no_grad()
def evaluate_ma2c_gnn_lstm_shared(
    args,
    actor: nn.Module,
    ...
    try:
        ...
        for eval_env in eval_env_pool.envs:
            ...
            while not done_flag:
                ...
                logits, rnn_hidden = actor.step(X, edge_index_cpu.to(device), hidden=rnn_hidden)     # [N,A]
...
# ---------------- training loop (single-episode updates; MA2C-PA + GNN+LSTM) ----------------
def run_training(args):
    ...
    # Build fixed grid topology once (kept on CPU; moved to device on use)
    edge_index_cpu = build_grid_edge_index(agent_id_list)  # [2,E], long

    # GNN+LSTM actor (decentralised over nodes) + per-agent GNN+LSTM critic
    ...
    for ep_idx in range(1, args.episodes + 1):
        ...
        while not done and steps < args.episode_steps:
            ...
            with torch.no_grad():
                logits_t, rnn_hidden = actor.step(obs_t, edge_index_cpu.to(device), hidden=rnn_hidden)  # [N,A]
        ...
        # Time-limit termination -> avoid bootstrapping past horizon
        done_seq[-1] = 1.0
        ...
        logs = ma2c_pa_update_gnn_lstm_attn(
            actor=actor,
            critic_pa=critic,
            ...
            done_seq=done_seq,
            last_obs=last_obs,
            edge_index=edge_index_cpu,           # pass graph
            edge_attr=None,                      # add edge features if you have them
            gamma=args.gamma,
        ...
        if ep_idx % args.eval_every == 0:
            eval_return = evaluate_ma2c_gnn_lstm_shared(args, actor, agent_id_list, edge_index_cpu, run_name=run_name)
...
# ------------------ CLI ------------------
def parse_args():
    parser = argparse.ArgumentParser("MA2C-PA (Per-Agent Central Critic, GNN+LSTM, single-episode updates)")
