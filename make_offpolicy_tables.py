...
VALUE_FONT_CMD = "\\fontsize{6.0pt}{6.3pt}\\selectfont"
...
def compute_kpi_widths_ex(
    ...
    # Keep the original compact KPI widths.
    # Do not widen them here, because wider columns can make subtables overlap.
...
def inner_tabular_with_grid(
    ...
    # - move KPI columns closer to Method using METHOD_TO_KPI_GAP
    # - widen gaps among TP / MTT / AWT using KPI_GAP
    # - keep a controlled gap between Stateless and LSTM-augmented blocks
    colspec = (
...
def build_2x2_single_table(
    ...
    lines.append("% =============================================")
    lines.append("% One table: 2x2 grid of off-policy results (mean ± std) with fixed KPI widths")
    lines.append("% =============================================")
    ...
    lines.append(
        "\\caption{Performance of off-policy (DQN-based) MARL policies across representative traffic demand scenarios. "
...
def main() -> None:
    ap = argparse.ArgumentParser(
        "Build ONE LaTeX table (2x2 grid) for off-policy evaluations from CSVs."
    )
