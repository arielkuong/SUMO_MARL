...
def build_color_map(labels):
    ...
    return {
        lbl: dark_palette[i % len(dark_palette)]
...
def main():
    parser = argparse.ArgumentParser(
        "Plot on-policy KPIs for multiple grid sizes in one figure, "
        "mean±std over seeds, with fixed-time and max-pressure baseline curves."
    )
    ...
    # -------------------- baseline arguments --------------------
    #
    # These defaults are the same baseline values used in the off-policy plot.
    #
    ...
    # Font for the two baseline entries.
    BASELINE_BOX_FS = 12
    ...
    # Width allocation.
    # Baseline has 2 entries, while stateless/stateful have 4 entries each.
    # Stateless and stateful are given equal width to make the row visually balanced.
    base_w = 0.17
    mem_w = 0.36
    ...
    # These settings are compact but still keep the entries readable.
