...
@torch.no_grad()
def evaluate_ma2c_shared(args, actor: nn.Module, agent_id_list: List[str], run_name: str) -> float:
...
def run_training(args):
    ...
    for ep_idx in range(1, args.episodes + 1):
        ...
        if ep_idx % args.eval_every == 0:
            eval_return = evaluate_ma2c_shared(args, actor, agent_id_list, run_name=run_name)
