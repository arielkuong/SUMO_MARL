...
from marl_utils.models import ActorMLP, CriticLSTM
from marl_utils.network_update import ia2c_update_mlp_actor_lstm_critic
...
@torch.no_grad()
def evaluate_ia2c_mlp_actor(args, actor: nn.Module, agent_id_list: List[str], run_name: str) -> float:
...
def run_training(args):
    ...
    # Shared MLP actor; shared LSTM critic (per-agent local value sequence)
    actor  = ActorMLP(O, A_dim, hidden=args.hidden).to(device)
    ...
    run_name = "ia2c_mlp_actor_lstm_critic"
    ...
    for ep_idx in range(1, args.episodes + 1):
        ...
        # last local obs for per-agent bootstrap
        last_obs_mat = np.stack([obs_dict[aid] for aid in agent_id_list], 0).astype(np.float32)
        last_obs = torch.as_tensor(last_obs_mat, dtype=torch.float32, device=device)  # [N,O]

        # -------- single update --------
        actor.train(); critic.train()
        logs = ia2c_update_mlp_actor_lstm_critic(
            actor=actor,
            critic=critic,
        ...
        if ep_idx % args.eval_every == 0:
            eval_return = evaluate_ia2c_mlp_actor(args, actor, agent_id_list, run_name=run_name)
...
def parse_args():
    parser = argparse.ArgumentParser("IA2C (MLP actor, LSTM critic, SHARED)")
