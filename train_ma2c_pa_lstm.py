...
from marl_utils.models import ActorLSTM, CriticPALSTM
from marl_utils.network_update import ma2c_pa_update_lstm
...
@torch.no_grad()
def evaluate_ma2c_lstm_shared(args, actor: nn.Module, agent_id_list: List[str], run_name: str) -> float:
    ...
    try:
        ...
        for eval_env in eval_env_pool.envs:
            ...
            while not done_flag:
                ...
                logits, lstm_hidden = actor.step(obs_tensor, lstm_hidden)                     # [N,A], (h,c)
...
# ---------------- training loop (single-episode updates; MA2C-PA + LSTM) ----------------
def run_training(args):
    ...
    # Shared decentralised actor (LSTM); central per-agent critic (LSTM on joint obs)
    actor  = ActorLSTM(obs_dim=O, act_dim=A_dim, hidden=args.hidden).to(device)
    critic = CriticPALSTM(joint_obs_dim=N * O, n_agents=N, hidden=args.hidden).to(device)

    optim_actor  = optim.Adam(actor.parameters(),  lr=args.lr_actor)
    optim_critic = optim.Adam(critic.parameters(), lr=args.lr_critic)

    run_name = "ma2c_pa_lstm"
    ...
    for ep_idx in range(1, args.episodes + 1):
        ...
        while not done and steps < args.episode_steps:
            ...
            with torch.no_grad():
                logits_t, lstm_hidden_actor = actor.step(obs_t, lstm_hidden_actor)   # [N,A]
        ...
        if ep_idx % args.eval_every == 0:
            eval_return = evaluate_ma2c_lstm_shared(args, actor, agent_id_list, run_name=run_name)
...
# ------------------ CLI ------------------
def parse_args():
    parser = argparse.ArgumentParser("MA2C-PA (Per-Agent Central Critic, LSTM, single-episode updates)")
    ...
    parser.add_argument('--normalize-rewards', type=str, default='per_agent', choices=['off','per_agent','global'],
                        help="Reward normalisation: per-agent over time, or global over T*N.")
