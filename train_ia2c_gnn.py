...
# NEW: import your GNN actor/critic + updater
from marl_utils.models import ActorGNNAttn, CriticGNNPerAgentAttn
from marl_utils.network_update import ia2c_update_gnn_attn

# helpers
from marl_utils.common import (
    set_global_seed,
    build_grid_edge_index,   # your helper that builds [2,E] from agent ids
    EvalHistory,
    clear_eval_history,
)
...
@torch.no_grad()
def evaluate_ia2c_gnn_attn(args, actor: nn.Module, agent_id_list: List[str],
    ...
    try:
        ...
        for eval_env in eval_env_pool.envs:
            ...
            while not done_flag:
                ...
                # NOTE: pass edge_index to actor forward
                logits = actor(X, edge_index_cpu.to(device))  # [N,A]
...
def run_training(args):
    ...
    # GNN actor (decentralized over nodes) + per-agent GNN critic
    ...
    for ep_idx in range(1, args.episodes + 1):
        ...
        # -------- single IA2C update (GNN) --------
        actor.train(); critic.train()
        logs = ia2c_update_gnn_attn(
            actor=actor,
            critic=critic,
            ...
            edge_attr=None,                  # or your edge features tensor if you add them
            gamma=args.gamma,
        ...
        if ep_idx % args.eval_every == 0:
            eval_return = evaluate_ia2c_gnn_attn(args, actor, agent_id_list, edge_index_cpu, run_name=run_name)
...
def parse_args():
    parser = argparse.ArgumentParser("IA2C (GNN-Attn, SHARED) — Train on Random Flows, Eval on Fixed Trips")
